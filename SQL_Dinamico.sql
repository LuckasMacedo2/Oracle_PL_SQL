...
/*
    Variaveis bind
        O Oracle possui uma arquitetura complexa de gerenciamento de memória
        Essa área de memória é chamada de shared pool que faz parte da SGA (System Global Area)
        Se o comando já está no shared pool o plano de execução para o comando é recuperado
        e o comando SQL é executado
        Caso o comando não está na shared pool o Oracle executa o comando de parsing do comando
        e escolhe o melhor plano de execução antes de executar o comando. Isso é chamado de hard parse
    Variavel bind precedida de : e é declarada fora do bloco pl/sql 
    Ao utilizá-la o Oracle não cria o plano de execução. 
    Isso é chamado de soft parse
    O Oracle escolhe o melhor plano de execução, ele busca evitar fazer o hard parse
    
    O SQL dinâmico pode utilizar variaveis bind para evitar o hard parse
*/
--
SET SERVEROUTPUT ON
SET VERIFY OFF
