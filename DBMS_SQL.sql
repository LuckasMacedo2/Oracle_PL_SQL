/*
    Package DBMS_SQL -> Package para SQL dinâmico
    O SQL dinâmico busca montar e executar um comando em tempo de execução
    
    As linhas retornadas por um cursor são colocadas em uma área de memória
    chamada de result set.
    
    Incluído da versão 7 / 8 sendo uma package legada
*/
...
BEGIN
  ...
  vrowcount := DBMS_SQL.EXECUTE_AND_FETCH(vcursor_id); -- Executa o select dinâmico e deixa no result set
